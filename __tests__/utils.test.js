...
test('Cash out commission calculation for natural persons', () => {
  ...
  expect(
    calcCashOutNaturalCommission(200, { last: 0, current: 200 }, config),
  ).toBe(0);
  expect(
    calcCashOutNaturalCommission(30000, { last: 0, current: 30000 }, config),
  ).toBe(87);
  expect(
    calcCashOutNaturalCommission(1000, { last: 30000, current: 31000 }, config),
  ).toBe(3);
  expect(
    calcCashOutNaturalCommission(100, { last: 31000, current: 31100 }, config),
  ).toBe(0.3);
  expect(
    calcCashOutNaturalCommission(100, { last: 31100, current: 31200 }, config),
  ).toBe(0.3);
});

test('Cash out commission calculation for juridical persons', () => {
  const config = { percents: 0.3, min: { amount: 0.5 } };
  expect(calcCashOutJuridicalCommission(200, config)).toBe(0.6);
  expect(calcCashOutJuridicalCommission(100, config)).toBe(0.5);
});
