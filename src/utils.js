...
const calcCashOutNaturalCommission = (amount, weeklyTotalsAmount, config, exceeded) => {
    if (weeklyTotalsAmount.current < config.week_limit.amount) return 0
    const amountToCommission = amount - (weeklyTotalsAmount.last < config.week_limit.amount ? config.week_limit.amount : 0)
